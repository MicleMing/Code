...
var cache = {
    ...
    register: function (name, getter) {
        var upperName = name.slice(0, 1).toUpperCase() + name.slice(1), getMethod = 'get' + upperName, setMethod = 'set' + upperName;
...
cache.register('ls', function (folder) {
    return fs.readdirSync(folder).map(function (name) {
        var p = path.join(folder, name), stat = cache.getStat(p);
        return {
            name: name,
            path: p,
            file: stat.isFile()
...
// config
var basePath = process.cwd(), commonPath = path.join(basePath, 'common'), pagePath = path.join(basePath, 'page'), distPath = path.join(basePath, '');
